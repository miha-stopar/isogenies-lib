...
#[cfg(test)]
mod tests {
    use crate::{ec_lit128, util::{big_to_bytes, Big}};

    #[test]
    fn dlog_3_test() {
        let curve = ec_lit128::Curve::new(&ec_lit128::Fq::ZERO);
        let fileName = "src/schemes/precomputed.json";
        let (P, _, _, _, _, _, power) = ec_lit128::load_torsion_info(fileName, "lit-sigamal-128", 3);
        ...
        for i in test_data {
            ...
            let dlog = ec_lit128::dlog_3(&curve, &Pa, &R, f);
            assert!(dlog == i);
        }
    }
    ...
    fn dlog_5_test() {
        let curve = ec_lit128::Curve::new(&ec_lit128::Fq::ZERO);
        let fileName = "src/schemes/precomputed.json";
        let (_, P, _, _, _, _, power) = ec_lit128::load_torsion_info(fileName, "lit-sigamal-128", 5);
        let (Pa, ok1) = curve.complete_pointX(&P);
        assert!(ok1 == 0xFFFFFFFF);
        ...
        for i in test_data {
            let k_bytes = big_to_bytes(i.clone());
            let R = curve.mul(&Pa, &k_bytes, k_bytes.len() * 8);
            let dlog = ec_lit128::dlog_5(&curve, &Pa, &R, f);
            assert!(dlog == i);
        }
    }
