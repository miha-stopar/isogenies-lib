...
macro_rules! define_fp2_core {
    () => {
        ...
        impl Fp2 {
            ...
            /// unsigned little-endian convention. Named simple because it trimms e
            /// and computes ebitlen.
            pub fn set_pow_simple(&mut self, e: &[u8]) {
                let mut zeros_at_end = 0;
                for b in e.iter().rev() {
                    if *b == 0 {
                        zeros_at_end += 1;
                    } else {
                ...
                let trimmed_bytes = &e[0..e.len() - zeros_at_end];
                let ebitlen = trimmed_bytes.len() * 8;

                self.set_pow_ext(trimmed_bytes, 0, ebitlen);
            }
        ...
        impl Zero for Fp2 {
            ...
            fn is_zero(&self) -> bool {
                self.equals(&Fp2::ZERO) == 0xFFFFFFFF
            }
        }
        ...
        impl One for Fp2 {
            ...
            #[inline]
            fn is_one(&self) -> bool {
                self.equals(&Fp2::ONE) == 0xFFFFFFFF
            }
        }
